...
class SmsEmailVerificationRepo {
  ...
  Future<bool> sendAuthorizationRequest() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (model.status == 'error') {
        CustomSnackbar.showCustomSnackbar(
            errorList: [model.message?.error ?? MyStrings.somethingWentWrong], msg: [], isError: false);
        return false;
      }
      return true;
    } else {
  ...
  Future<bool> resendVerifyCode({required bool isEmail}) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (model.status == 'error') {
        CustomSnackbar.showCustomSnackbar(
            errorList: [model.message?.error ?? MyStrings.resendCodeFail], msg: [], isError: true);
        return false;
      } else {
        CustomSnackbar.showCustomSnackbar(
            errorList: [''], msg: model.message?.success ?? [MyStrings.successfullyCodeResend], isError: false);
        return true;
      }
    } else {
      return false;
