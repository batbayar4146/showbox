...
import 'package:showbox/data/model/subscribe_plan/buy_subscribe_plan_response_model/BuySubscribePlanResponseModel.dart';
import 'package:showbox/data/model/subscribe_plan/subscribe_plan_response_model/SubscribePlanResponseModel.dart';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (bModel.status == 'success') {
          ...
          update();
          final res = Get.toNamed(RouteHelper.depositScreen,
              arguments: [planList[index].pricing, planList[index].name, planList[index].id.toString()]);
        } else {
          CustomSnackbar.showCustomSnackbar(
              errorList: [bModel.message?.error?.toString() ?? 'Failed to buy subscribe plan'],
              msg: [''],
              isError: true);
        }
      } else {
