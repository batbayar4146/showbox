...
class AddNewDepositController extends GetxController implements GetxService {
  ...
  setPaymentMethod(Methods? method) {
    ...
    depositLimit =
        'Deposit Limit: ${CustomValueConverter.twoDecimalPlaceFixedWithoutRounding(method?.minAmount?.toString() ?? '-1')} - ${CustomValueConverter.roundDoubleAndRemoveTrailingZero(method?.maxAmount?.toString() ?? '-1')} $currency';
    charge =
  ...
  void submitDeposit(String price, String subscriptionId) async {
    ...
    InsertDepositResponseModel mo =
        await depositRepo.insertDeposit(amount1, paymentMethod?.methodCode, paymentMethod?.currency, subscriptionId);
