...
import 'package:showbox/data/repo/deposit_repo/deposit_repo.dart';
import 'package:showbox/view/screens/account/payment_log_screen/widget/payment_log_list_item/payment_log_list_item.dart';
        ...
        builder: (controller) => ListView.builder(
            ...
            itemBuilder: (builder, index) {
              if (index == controller.depositList.length) {
                return Center(
                  child: controller.hasNext() ? const CircularProgressIndicator() : const SizedBox(),
                );
              }
              return DepositHistoryListItem(listItem: controller.depositList[index], index: index, currency: currency);
            }),
      ),
    );
  }

  Widget buildRow({required String header, required String value, bool isDetails = false}) {
    return Column(
      children: [
        const SizedBox(
          height: 8,
